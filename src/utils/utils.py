...
from .datasets import (MNIST_truncated, MNIST_rotated, CIFAR10_truncated, CIFAR10_rotated, CIFAR100_truncated, 
                       SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, 
                       Generated, genData, ImageFolder_custom, USPS_truncated, GTSRB_truncated, MIOTCD_truncated, 
                       Vehicle10_truncated, StanfordCars_truncated, TLight10_truncated)
...
def load_stanford_cars_data(datadir, resize=(224, 224)):
    trans = [transforms.ToTensor()]
    if resize:
        trans.insert(0, transforms.Resize(resize))
    trans = transforms.Compose(trans)

    stanford_cars_train_ds = StanfordCars_truncated(root=datadir, train=True, transform=trans, download=False)
    stanford_cars_test_ds = StanfordCars_truncated(root=datadir, train=False, transform=trans, download=False)

    X_train, y_train = stanford_cars_train_ds.data, stanford_cars_train_ds.target
    X_test, y_test = stanford_cars_test_ds.data, stanford_cars_test_ds.target
...
def load_miotcd_data(datadir, resize=(32,32)):
    trans = [transforms.ToTensor()]
    if resize:
        trans.insert(0, transforms.Resize(resize))
    trans = transforms.Compose(trans)

    miotcd_train_ds = MIOTCD_truncated(root=datadir, train=True, transform=trans, download=False)
    miotcd_test_ds = MIOTCD_truncated(root=datadir, train=False, transform=trans, download=False)

    X_train, y_train = miotcd_train_ds.data, miotcd_train_ds.target
    X_test, y_test = miotcd_test_ds.data, miotcd_test_ds.target
...
def load_gtsrb_data(datadir, resize=(32,32)):
    trans = [transforms.ToTensor()]
    if resize:
        trans.insert(0, transforms.Resize(resize))
    trans = transforms.Compose(trans)
    
    gtsrb_train_ds = GTSRB_truncated(root=datadir, train=True, transform=trans, download=True)
    gtsrb_test_ds = GTSRB_truncated(root=datadir, train=False, transform=trans, download=True) 
    
    X_train, y_train = gtsrb_train_ds.data, gtsrb_train_ds.target
    X_test, y_test = gtsrb_test_ds.data, gtsrb_test_ds.target
...
def load_tlight10_data(datadir, resize=(32,32)):
    trans = [transforms.ToTensor()]
    if resize:
        trans.insert(0, transforms.Resize(resize))
    trans = transforms.Compose(trans)

    tlight10_train_ds = TLight10_truncated(root=datadir, train=True, transform=trans, download=False)
    tlight10_test_ds = TLight10_truncated(root=datadir, train=False, transform=trans, download=False)

    X_train, y_train = tlight10_train_ds.data, tlight10_train_ds.target
    X_test, y_test = tlight10_test_ds.data, tlight10_test_ds.target
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, dataidxs_test=None,
    ...
    if dataset in ('mnist', 'mnist_rotated', 'femnist', 'fmnist', 'cifar10', 'cifar10_rotated', 'cifar100',
                   ...
                   'gtsrb', 'miotcd', 'vehicle10', 'stanford_cars', 'tlight10'):

        if dataset == 'stanford_cars':
            dl_obj = StanfordCars_truncated
        ...
        elif dataset == 'mnist' or dataset == 'mnist_rotated':
            if dataset == 'mnist':
