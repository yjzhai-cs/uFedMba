...
import numpy as np
from torchvision.datasets import MNIST, CIFAR10, SVHN, FashionMNIST, CIFAR100, ImageFolder, DatasetFolder, USPS, GTSRB, StanfordCars
...
class StanfordCars_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        stanfordcars_dataobj = StanfordCars(root=self.root, split=split, transform=self.transform, download=False)
        data, target = [], []

        if self.dataidxs is None:
            self.dataidxs = [i for i in range(len(stanfordcars_dataobj))]
        
        for idx in self.dataidxs:
            data.append(stanfordcars_dataobj[idx][0])
            target.append(stanfordcars_dataobj[idx][1])
...
class Vehicle10Dataset(data.Dataset): 
    def __init__(self, 
                root: str,
                split: str = 'train',
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(img_path).convert('RGB')     # 像素值 0~255，在transfrom.totensor会除以255，使像素值变成 0~1   参考：https://blog.csdn.net/icamera0/article/details/50843172
...
class Vehicle10_truncated(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
...
class MIOTCDDataset(data.Dataset): 
    def __init__(self, 
                root: str,
                split: str = 'train',
                transform: Optional[Callable] = None,
        ...
        self.meta_info = None
        if split == 'train':
            with open(os.path.join(root, 'mio-tcd', 'train_meta.json'), 'r') as file:
                self.meta_info = json.load(file)
        elif split == 'test':
            with open(os.path.join(root, 'mio-tcd', 'valid_meta.json'), 'r') as file:
                self.meta_info = json.load(file)
        else:
            raise RuntimeError(f'{split} is undefined')
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.root, 'mio-tcd', self.meta_info['path'][index])
        img = Image.open(img_path).convert('RGB')     # 像素值 0~255，在transfrom.totensor会除以255，使像素值变成 0~1   参考：https://blog.csdn.net/icamera0/article/details/50843172
...
class MIOTCD_truncated(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
    ...
    def __build_truncated_dataset__(self,):
        ...
        miotcd_dataobj = MIOTCDDataset(root=self.root, split=split, transform=self.transform, target_transform=self.target_transform)
        data, target = [], []
        if self.dataidxs is None:
            self.dataidxs = [i for i in range(len(miotcd_dataobj))]

        for idx in self.dataidxs:
            data.append(miotcd_dataobj[idx][0])
            target.append(miotcd_dataobj[idx][1])
...
class GTSRB_truncated(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
    ...
    def __build_truncated_dataset__(self,):
        ...
        gtsrb_dataobj = GTSRB(root=self.root, split=split, transform=self.transform, target_transform=self.target_transform, download=self.download)
        data, target = [], []
        if self.dataidxs is None:
            self.dataidxs = [i for i in range(len(gtsrb_dataobj))]
        
        for idx in self.dataidxs:
            data.append(gtsrb_dataobj[idx][0])
            target.append(gtsrb_dataobj[idx][1])
...
class TLight10Dataset(data.Dataset): 
    def __init__(self, 
                root: str,
                split: str = 'train',
                transform: Optional[Callable] = None,
        ...
        self.meta_info = None
        if split == 'train':
            with open(os.path.join(root, 'tlight-10', 'train_meta.json'), 'r') as file:
                self.meta_info = json.load(file)
        elif split == 'test':
            with open(os.path.join(root, 'tlight-10', 'valid_meta.json'), 'r') as file:
                self.meta_info = json.load(file)
        else:
            raise RuntimeError(f'{split} is undefined')
    ...
    def __getitem__(self, index):
        img_path = os.path.join(self.root, 'tlight-10', self.meta_info['path'][index])
...
class TLight10_truncated(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
    ...
    def __build_truncated_dataset__(self,):
        ...
        tlight10_dataobj = TLight10Dataset(root=self.root, split=split, transform=self.transform, target_transform=self.target_transform)
        data, target = [], []
        if self.dataidxs is None:
            self.dataidxs = [i for i in range(len(tlight10_dataobj))]

        for idx in self.dataidxs:
            data.append(tlight10_dataobj[idx][0])
            target.append(tlight10_dataobj[idx][1])
