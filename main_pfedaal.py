...
for idx in range(args.num_users):
    ...
    clients.append(Client_pFedAAL(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep, 
               args.lr, args.momentum, args.device, args.L, args.mu, train_dl_local, test_dl_local))


##################################### Split the model
based_layers_name, personalized_layers_name = get_based_and_personalzied_layers_name(net_glob, args.L)
...
clients_history_loss = [[] for _ in range(args.num_users)]
clients_history_personalized_layers_params = [[] for _ in range(args.num_users)]
...
for iteration in range(args.rounds):
    ...
    for idx in idxs_users:
        ...
        delta, loss = clients[idx].train(is_print=False)

        clients_delta_list.append(copy.deepcopy(delta)) 
        loss_locals.append(copy.deepcopy(loss))
        clients_history_loss[idx].append(copy.deepcopy(loss))
    ...
    client_model_based_layers_params_list = []
    client_model_personalized_layers_params_list = []

    # split the model
    for idx in idxs_users:
        based_params = []
        personalized_params = []
        for n, v in clients[idx].net.state_dict().items():
            if n in based_layers_name:
                based_params.append(v)
            
            if n in personalized_layers_name:
                personalized_params.append(v)

        client_model_based_layers_params_list.append(list(based_params))
        client_model_personalized_layers_params_list.append(list(personalized_params))
        clients_history_personalized_layers_params[idx].append(list(personalized_params))

    # aggregate the global base model
    ...
    weights = torch.tensor(fed_avg_freqs, device=args.device)

    aggregated_based_layers_params = aggregate_parameters(client_model_based_layers_params_list, weights)
    ...
    #         client_personalized_params = aggregate_parameters(history_personalized_params, scores)
        ...
        temp_clients_model_personalized_layers_params_list = client_model_personalized_layers_params_list[:index]
        if index + 1 < len(clients_delta_list):
            temp_clients_delta_list = temp_clients_delta_list + clients_delta_list[index + 1:]
            temp_clients_model_personalized_layers_params_list = temp_clients_model_personalized_layers_params_list + client_model_personalized_layers_params_list[index + 1:]
        
        gamma_matrix = []
        for layer_name in personalized_layers_name:
            gamma_list = []
            total = 0
            for delta in temp_clients_delta_list:
                beta = compute_cos(clients_delta_list[index][layer_name], delta[layer_name]) + 1
                total += beta
                gamma_list.append(beta)
        
            gamma_list = torch.tensor(gamma_list, device=args.device)
            gamma_list = gamma_list / total
        ...
        all_layers_parameters = copy.deepcopy(aggregated_based_layers_params + personalized_layers_parameters)

        # set the new parameters to client
        params_dict = OrderedDict(
            zip(clients[idx].net.state_dict().keys(), all_layers_parameters)
        )
        clients[idx].set_state_dict(params_dict)
